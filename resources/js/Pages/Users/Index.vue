                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('fields.email') }}</th>
                                                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">{{ __('fields.role') }}</th>
                                            ...
                                            <tr v-for="user in users.data" :key="user.id">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ user.created_at }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <Link :href="route('users.show', user)" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150 mr-1">
                                                        {{ __('common.show') }}
                                                    </Link>
                                                    <Link :href="route('users.edit', user)" v-if="permissions.edit" class="inline-flex items-center px-4 py-2 bg-white border border-blue-500 rounded-md font-semibold text-xs text-blue-500 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150 mr-1">
                                                        {{ __('common.edit') }}
                                                    </Link>
                                                    <Link :href="route('users.destroy', user)" method="DELETE" as="button" class="inline-flex items-center px-4 py-2 bg-red border border-red-300 rounded-md font-semibold text-xs text-red-500 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150 mr-1">
                                                        {{ __('common.delete') }}
                                                    </Link>
                                                </td>
                                            </tr>
